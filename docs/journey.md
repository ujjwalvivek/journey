...
A custom high-performance 2D ECS game engine written in Rust + WGPU. Features AABB physics, focuses on precision platforming (*Hollow Knight*) and parry-based combat (*Sekiro*/*Nine Sols*). For a Metroidvania running at 60FPS (`Important Metric`) in a web browser, I want tight, deterministic, arcade physics, not realistic simulations. This project also serves as a "Living Proof of Work" for a **TPM** role.
...
- **Inspiration:** Hollow knight Series, Nine Sols, Sekiro, Dead Cells, Celeste, Blasphemous.